...
(function () {
  ...
  function initBreathBar() {
    ...
    function setRunning(state) {
      ...
      toggle.textContent = state ? 'Pause the pacer' : 'Start the pacer';
      if (state) {
        start = null;
        raf = window.requestAnimationFrame(frame);
      } else if (raf) {
  ...
  function initSequencePlayer() {
    ...
    function stop() {
      ...
      startBtn.textContent = 'Begin the practice';
    ...
    function advance(step) {
      ...
      if (next >= items.length) {
        ...
        remaining = 0;
        paint();
        nameEl.textContent = 'That is the whole practice';
        sideEl.textContent = 'Stay down as long as you like before you stand up.';
        cueEl.textContent = 'Let the breath settle on its own before the day starts again.';
        return;
      }
      idx = next;
  ...
  function initForms() {
    $$('form[data-validate]').forEach(function (form) {
      ...
      function messageFor(input) {
        if (input.validity.valueMissing) return input.dataset.msgRequired || 'We need this one to go on.';
        if (input.validity.typeMismatch && input.type === 'email') return 'That does not look like an email address yet. Try the name@example.com shape.';
        if (input.validity.tooShort) return 'Please use at least ' + input.minLength + ' characters.';
        if (input.validity.patternMismatch) return input.dataset.msgPattern || 'Something in the format is off. Have another look.';
      ...
      form.addEventListener('submit', function (e) {
        ...
        if (status) {
          status.textContent = form.dataset.successMessage ||
            'Thank you. Your note has arrived and a person reads every one. We reply within two business days.';
  ...
  /* ------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
  var CONSENT_KEY = 'stillform-consent-v1';
